  ...
  const fetchLeaderboard = useCallback(async () => {
    try {
      ...
      if (error) {
        ...
        const mappedPlayers = (data as LeaderboardProfile[]).map((user) => ({
          id: user.id,
      ...
      <div className="container mx-auto px-4 max-w-5xl relative z-10">
        ...
        <div className="text-center mb-12 space-y-4">
          <motion.h2 variants={fadeUp} className="text-3xl md:text-5xl font-bold text-center mb-4">
             <span className="text-accent-gradient">Leaderboard</span>
          </motion.h2>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-slate-400 text-lg max-w-xl mx-auto">
            Updates automatically as soon as points are assigned. No reload needed.
          </motion.p>
        </div>
