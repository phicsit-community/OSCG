...
interface GlobeVibrantProps {
  className?: string;
}

export function GlobeVibrant({ className }: GlobeVibrantProps) {
  ...
  useEffect(() => {
    ...
    controls.autoRotateSpeed = 1.2; // Faster spin for signup page
    // Lock the polar angle for consistent view
    ...
    new THREE.TextureLoader().load(
      ...
      (earthTexture: THREE.Texture) => {
        ...
        // Fragment Shader - VIBRANT GREEN theme
        const fragmentShader = `
          ...
          void main() {
            ...
            // Vibrant green color palette - more saturated and brighter
            float t = (sin(u_time * 0.8) + 1.0) / 2.0;
            vec3 brightGreen = vec3(0.07, 0.92, 0.55);  // #11EB8C - very bright green
            vec3 emerald = vec3(0.0, 1.0, 0.65);        // #00FFA6 - neon green
            vec3 teal = vec3(0.2, 0.95, 0.85);          // More vibrant teal
            
            // Multi-color gradient shift
            vec3 color1 = mix(brightGreen, emerald, t);
            vec3 color2 = mix(emerald, teal, t);
            vec3 shiftColor = mix(color1, color2, sin(u_time * 0.3) * 0.5 + 0.5);

            float dot = 1. - smoothstep(.38, .4, length(gl_PointCoord.xy - vec2(.5)));
            if (dot < 0.5) discard;

            // Add glow effect
            float glow = 1.2 + 0.3 * sin(u_time * 1.5);
            
            gl_FragColor = vec4(baseColor * shiftColor * glow, dot * vOpacity);
          }
        `;
        ...
        function updateSize() {
          ...
          mapMaterial.uniforms.u_dot_size.value = 0.045 * minSide; // Slightly larger dots
        }
        updateSize();
        ...
        canvas3D.addEventListener("click", (event) => {
          ...
          if (intersects.length > 0) {
            ...
            popupEl.style.background = "linear-gradient(135deg, #11EB8C, #00FFA6)";
            popupEl.style.padding = "6px 12px";
            popupEl.style.borderRadius = "8px";
            popupEl.style.fontSize = "14px";
            popupEl.style.fontWeight = "600";
            popupEl.style.boxShadow = "0 0 20px rgba(17, 235, 140, 0.5)";
